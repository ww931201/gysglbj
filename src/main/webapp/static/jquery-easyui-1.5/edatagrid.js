			...
			onAfterEdit: function(index, row){
				...
				if (url){
					...
					if (changed){
							var newRow = {};
							for ( var p in row) {
								if(p != 'ROWNUM_' && p != 'isNewRecord' && p != 'theme') {						
									newRow[opts.modelName+"." + p] = row[p];
								}else{
									newRow[p] = row[p];
								}
							}
						opts.poster.call(target, url, newRow, function(data){
							if (data.isError){
			...
			return jq.each(function(){
				...
				function _del(row){
					...
					if (row.isNewRecord){
						...
						if (opts.destroyUrl){
							var idValue = row[opts.idField||'id'];
							opts.poster.call(dg[0], opts.destroyUrl, {id : idValue}, function(data){//改ID名称
