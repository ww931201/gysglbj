	...
	function parseToTree(array){
		...
		for(var i=0; i<array.length; i++){
			if(array[i].id == array[i].parentId) {
				arrbest.push(array[i]);
			}else {
				arrOther.push(array[i]);				
			}
		}
//		console.log("第一步: 找出主菜单列表");
//		console.log(arrbest);
		for(var i=0; i<arrbest.length; i++) {			
			var obj = loadTree(arrbest[i], arrOther)
			arrbest[i] = obj;
		}
		return arrbest;
	}
	function loadTree(data, arrOther) {
		var temp = new Array();
		for(var i=0; i<arrOther.length; i++){
			if(arrOther[i].parentId == data.id && arrOther[i].id != arrOther[i].parentId) {
				temp.push(loadTree(arrOther[i], arrOther));
			}
		}
		data.children = temp;
		return data;
	}
	//console.log(parseToTree(data));
_1cd(parseToTree(data));
//_1cd(data);
},error:function(){
