#namespace("user")
	...
	### 全部用户(超级管理员)
	#sql("paginateSelect")
		select ID, USER_NAME, USER_CODE, ROLE_NAME, REMARK, ENTRY_TIME from VIEW_DIC_USER WHERE 1=1 
		#if(USER_NAME ??)
	...
	#sql("paginateSelectChild")
		select ID, USER_NAME, USER_CODE, ROLE_NAME, REMARK, ENTRY_TIME from VIEW_DIC_USER WHERE PARENT_ID = '#(PARENT_ID)'
		#if(USER_NAME ??)
			and USER_NAME like '%#(USER_NAME)%'
		#end
		#if(USER_CODE ??)
	...
	### 判断登录用户是否存在
	#sql("isPassed")
		select * from VIEW_DIC_USER where USER_CODE = ? AND PASSWD = ?
	#end
